...
<User className="h-10 w-10 text-teal dark:text-teal-light" aria-hidden="true" />
</div>

<Card className="w-full bg-card border-border shadow-xl backdrop-blur-sm">
<CardHeader>
...
<TabsList className="grid grid-cols-2 bg-muted mb-4 px-[3px]">
...
{error && <div className="mx-6 mb-4 p-3 bg-red-100 dark:bg-red-900/40 border border-red-300 dark:border-red-800 rounded-md flex items-center gap-2 text-red-700 dark:text-red-200" role="alert">
...
<Label htmlFor="email" className="text-foreground text-base">{t("auth.email")}</Label>
<div className="relative">
<Mail className="absolute left-3 top-1/2 transform -translate-y-1/2 h-5 w-5 text-muted-foreground" aria-hidden="true" />
<Input id="email" type="email" placeholder="your.email@example.com" value={email} onChange={e => setEmail(e.target.value)} required className="pl-10 py-5 bg-background border-border text-foreground text-base" />
</div>
</div>

<div className="space-y-2">
<Label htmlFor="password" className="text-foreground text-base">{t("auth.password")}</Label>
<div className="relative">
<Lock className="absolute left-3 top-1/2 transform -translate-y-1/2 h-5 w-5 text-muted-foreground" aria-hidden="true" />
<Input id="password" type="password" placeholder="••••••••" value={password} onChange={e => setPassword(e.target.value)} required className="pl-10 py-5 bg-background border-border text-foreground text-base" />
</div>
</div>
</CardContent>
...
<CardContent className="space-y-4 mt-2 pb-4">
<div className="space-y-2">
<Label htmlFor="signup-email" className="text-foreground text-base">{t("auth.email")}</Label>
<div className="relative">
<Mail className="absolute left-3 top-1/2 transform -translate-y-1/2 h-5 w-5 text-muted-foreground" aria-hidden="true" />
<Input id="signup-email" type="email" placeholder="your.email@example.com" value={email} onChange={e => setEmail(e.target.value)} required className="pl-10 py-5 bg-background border-border text-foreground text-base" />
</div>
</div>

<div className="space-y-2">
<Label htmlFor="signup-password" className="text-foreground text-base">{t("auth.password")}</Label>
<div className="relative">
<Lock className="absolute left-3 top-1/2 transform -translate-y-1/2 h-5 w-5 text-muted-foreground" aria-hidden="true" />
<Input id="signup-password" type="password" placeholder="••••••••" value={password} onChange={e => setPassword(e.target.value)} required className="pl-10 py-5 bg-background border-border text-foreground text-base" />
</div>
</div>
</CardContent>
