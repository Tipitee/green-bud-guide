...
<Button onClick={() => setShowNewEntryModal(true)} className="bg-card hover:bg-accent/20 text-teal border border-teal/30 shadow-sm rounded-md">
...
<div className="space-y-4" role="list" aria-label={t('journal.existingEntries')}>
{entries.map(entry => <Card key={entry.id} className="journal-entry card-rounded" role="listitem">
...
<Textarea id="notes" name="notes" value={editedEntry?.notes || ""} onChange={handleEditInputChange} className="input-field form-control bg-background" />
</div>
<div className="flex justify-end gap-2">
<Button onClick={cancelEditing} variant="outline" className="border-border text-foreground bg-card rounded-md">
<X size={16} className="mr-1" aria-hidden="true" />
{t('common.cancel')}
</Button>
<Button onClick={saveEntry} className="text-teal border border-teal/30 shadow-sm bg-card hover:bg-accent/10 rounded-md">
{t('journal.save')}
</Button>
...
{entry.activity && <span className="text-muted-foreground">
...
{entry.sideEffects.map(effect => <Badge key={effect} variant="outline" className="bg-muted text-xs">
...
<Button onClick={() => startEditing(entry.id)} variant="secondary" size="sm" className="rounded-md" aria-label={t('journal.edit')}>
