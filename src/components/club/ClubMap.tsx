...
const MAP_STYLE: any = { version: 8, sources: { osm: { type: "raster", tiles: ["https://tile.openstreetmap.org/{z}/{x}/{y}.png"], tileSize: 256, attribution: "© OpenStreetMap contributors", maxzoom: 19 } }, layers: [{ id: "osm-tiles", type: "raster", source: "osm" }] };

interface ClubMapProps { club?: ClubResult; allClubs?: ClubResult[]; }
...
const ClubMap: React.FC<ClubMapProps> = ({ club, allClubs }) => {
  ...
  // Initialize map once — set isMapReady after tiles load
  useEffect(() => {
    if (!mapContainer.current || mapRef.current) return;
    const m = new maplibregl.Map({ container: mapContainer.current, style: MAP_STYLE, center: DEFAULT_CENTER, zoom: DEFAULT_ZOOM });
    mapRef.current = m;
    m.once("load", () => setIsMapReady(true));
    return () => { m.remove(); mapRef.current = null; setIsMapReady(false); };
  }, []);
  ...
  // Add/update markers ONLY when map is ready — eliminates race condition
  useEffect(() => {
    ...
    if (club && club.latitude && club.longitude) {
      const mk = new maplibregl.Marker({ color: "#14b8a6" })
        ...
        .setPopup(new maplibregl.Popup({ offset: 25 }).setHTML("<strong>" + club.name + "</strong><br/><small>" + (club.address || "") + "</small>"))
        .addTo(m);
    ...
    valid.forEach(c => {
      ...
      const popup = new maplibregl.Popup({ closeButton: true, offset: 25, maxWidth: "200px" }).setHTML(
        "<div style=\"font-family:sans-serif;padding:2px\"><div style=\"font-weight:600;margin-bottom:2px\">" + c.name + "</div>"
        + "<div style=\"font-size:12px;color:#555\">" + (c.city || "") + (c.city && c.postal_code ? ", " : "") + (c.postal_code || "") + "</div>"
        + "<button id=\"" + safeId + "\" style=\"font-size:12px;color:#14b8a6;cursor:pointer;margin-top:6px;background:none;border:none;padding:0\">Voir détails →</button></div>"
      );
      popup.on("open", () => { const btn = document.getElementById(safeId); if (btn) btn.onclick = () => navigateRef.current("/clubs/" + encodeURIComponent(c.name)); });
      const mk = new maplibregl.Marker({ element: el }).setLngLat([c.longitude!, c.latitude!]).setPopup(popup).addTo(m);
      markersRef.current.push(mk);
      bounds.extend([c.longitude!, c.latitude!]);
    });
    if (valid.length > 0) m.fitBounds(bounds, { padding: 50, maxZoom: 12, duration: 500 });
  }, [isMapReady, club, allClubs]);
  ...
  return (
    <>
      ...
      <div className="absolute right-4 bottom-4 z-10">
        <Button onClick={getCurrentLocation} disabled={isLoadingLocation} variant="secondary" size="icon" className="h-10 w-10 rounded-full shadow-lg bg-white dark:bg-navy-400 hover:bg-gray-100">
          {isLoadingLocation ? <Loader2 className="h-5 w-5 text-teal dark:text-teal-light animate-spin" /> : <MapPin className="h-5 w-5 text-teal dark:text-teal-light" />}
        </Button>
      </div>
    </>
  );
